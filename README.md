...
- 管理后台主页：
  打开 `http://47.101.38.217:18080/` 后，必须先通过 **全页 token 门禁** 验证，验证成功后才会渲染主页界面。
- 分享链接页：
  形如 `http://47.101.38.217:18080/?share_token=...` 的分享页 **不需要主页 token**，可直接进入页面并完成扫码登录、填写购票人和提交操作。
...
如果后端启动时配置了 `--token`，现在前端会先显示一个 **全页 token 门禁页**，而不是弹出浏览器提示框。

- 本地后端**未配置** `--token`：
  会直接进入主页。
- 本地后端**配置了** `--token`：
  先输入 token，验证通过后再进入主页。

分享页 `?share_token=...` 仍然不会经过这个主页门禁。
